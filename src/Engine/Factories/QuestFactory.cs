    ...
    internal static class QuestFactory
    {
        ...
        private static void LoadQuestsFromNodes(XmlNodeList nodes)
        {
            foreach (XmlNode node in nodes)
            {
                ...
                {
                    itemsToComplete.Add(new ItemQuantity(childNode.AttributesAsInt("ID"),
                                                         childNode.AttributesAsInt("Quantity")));
                }
                foreach (XmlNode childNode in node.SelectNodes("./RewardItems/Item"))
                {
                    rewardItems.Add(new ItemQuantity(childNode.AttributesAsInt("ID"),
                                                     childNode.AttributesAsInt("Quantity")));
                }
                _quests.Add(new Quest(node.AttributesAsInt("ID"),
                                      node.SelectSingleNode("./Name")?.InnerText ?? "",
                                      node.SelectSingleNode("./Description")?.InnerText ?? "",
                                      itemsToComplete,
                                      node.AttributesAsInt("RewardExperiencePoints"),
                                      node.AttributesAsInt("RewardGold"),
                                      rewardItems));
            }
        }
