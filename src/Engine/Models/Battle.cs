...
namespace Engine.Models
{
    public class Battle : IDisposable
    {
        ...
        public Battle(Player player, Monster opponent)
        {
            ...
            if(CombatService.FirstAttacker(_player, opponent) == CombatService.Combatant.Opponent)
            {
                AttackPlayer();
            }
        }
        ...
        private void OnOpponentKilled(object sender, System.EventArgs e)
        {
            ...
            _messageBroker.RaiseMessage($"You recieve {_opponent.RewardExperiencePoints} experience points.");
            _player.AddExperiencePoints(_opponent.RewardExperiencePoints);

            _messageBroker.RaiseMessage($"You recieve {_opponent.Gold} gold.");
            _player.RecieveGold(_opponent.Gold);

            foreach(GameItem gameItem in _opponent.Inventory)
            {
                _messageBroker.RaiseMessage($"You recieve one {gameItem.Name}.");
